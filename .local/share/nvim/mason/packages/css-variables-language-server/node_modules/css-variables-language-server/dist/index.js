...
function culoriColorToVscodeColor(color) {
  ...
  return { red: rgb.r, green: rgb.g, blue: rgb.b, alpha: (_a = rgb.alpha) != null ? _a : 1 };
}

// src/utils/resolveVariableValue.ts
var MAX_DEPTH = 5;
function parseVarReference(varRef) {
  var _a;
  ...
  return {
    varName: match[1],
    fallback: (_a = match[2]) == null ? void 0 : _a.trim()
  };
}
function resolveVariableValue(value, variableMap, depth = 0, visited = /* @__PURE__ */ new Set()) {
  if (depth >= MAX_DEPTH) {
    return value;
  }
  ...
  resolveAllVariableReferences() {
    ...
    allVariables.forEach((cssVariable, varName) => {
      ...
      if (resolvedValue !== originalValue) {
        let culoriColor;
        try {
          culoriColor = culori3.parse(resolvedValue);
        } catch {
          return;
        }
        if (culoriColor) {
